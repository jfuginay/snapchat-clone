-- Safe Database Setup for Snapchat Clone
-- This script ONLY adds custom application tables and does NOT modify existing auth schema
-- Run this in your Supabase SQL Editor

-- Only drop our custom tables if they exist (NOT auth tables)
...
-- Create custom users table (extends the existing auth.users)
...
-- Enable Row Level Security (RLS) for our custom tables only
...
-- Enable real-time subscriptions for our custom tables
-- First remove from publication if they exist (ignore errors)
DO $$
BEGIN
  BEGIN
...
-- Add our custom tables to real-time publication
...
DO $$
BEGIN
  RAISE NOTICE 'Snapchat Clone database setup completed successfully!';
  RAISE NOTICE 'Added custom tables: users, locations, friendships, chat_rooms, messages, photos';
  RAISE NOTICE 'Your existing auth schema is untouched and safe.';
END $$; 